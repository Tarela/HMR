...
#list.of.packages <- c("glmnet","foreach")
#new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
#if(length(new.packages)) install.packages(new.packages,dependencies=TRUE,lib=tmp_rpackage_dir,repos="https://mirrors.tongji.edu.cn/CRAN/")
#
...
#print(dim(coTF_binding_matrix))
#print((all_use_coTF_names))
...
if(noTFdetected==1){  
    write.table("no non-classic function detected",file=paste0(outname,"_NCfull.txt"),quote=F,sep="\t",row.names=F,col.names=F)
    ...
    write.table(summary_table_raw,file=paste0(outname,"_NCfull.txt"),quote=F,sep="\t",row.names=T,col.names=T)
...
#signal2cutoff_backup <- function(rawsig){
#    # function for separate NC peaks considering HMsignal
#    # input data: a vector of signal (HMsignal on CR peak, linear scale)
#    # method: go through all possible cutoff, find the cutoff corresponding to maximum G
#    # output: the cutoff, a vector of selected cutoff candidates (Gbins) and a vector of G value for each cutoff candidates (G) 
