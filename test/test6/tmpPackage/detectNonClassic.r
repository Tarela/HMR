...
if(is.null(summary_table)){
    ...
    for(topnum in 1:nrow(out_table_raw)){
        ...
        if(topnum %in% 1:4){
            ## boxplot compare the histone modification signal between on classic and non-classic peaks
            boxplot(Y[which(coTF_cobinding>0 & HMnc == 0)],Y[which(coTF_cobinding==0 | HMnc > 0)],
                names=c("non-classic peak","classic peak"),ylab=paste0(substrateHM," signal"),main=paste0(coTF," & ",substrateHM),
